...
static UINT32 UARTTXBUFSIZE[UART_NUM] = {64, 64, 512, 64, 4096, 64, 64, 64, 4096, 64, 4096};  /* UART0~10 Tx buffer size */
static UINT32 UARTRXBUFSIZE[UART_NUM] = {64, 64, 512, 128*1024, 4096, 64, 64, 128*1024, 4096, 64, 4096};    /* UART0~10 Rx buffer size */
/*
...
// by sam
static SemaphoreHandle_t xuartSemaphore[UART_NUM];
...
void _uartTransmitChars(INT nNum)
{
    UINT32 volatile i;
    ...
    if(dev->uUartTxHead != dev->uUartTxTail) 
    { /* buffer is not empty */
        for (i=0; i<8; i++) 
        {
...
static void _uartCheckModemStatus(INT nNum)
{
    ...
    {
        if(!(uRegMSR & 0x10)) 
        { /* CTS high, external signal is low */
    ...
    {
        if( !(uRegMSR & 0x10) ) 
        { /* CTS low, external signal is high */
...
static BOOL _uartBUFSpaceAlloc(INT nNum)
{
    ...
    //sysprintf("#### _uartBUFSpaceAlloc TX [%d]:0x%08x...\r\n", nNum, dev->pucUartTxBuf); 
    ...
    //sysprintf("#### _uartBUFSpaceAlloc RX [%d]:0x%08x...\r\n", nNum, dev->pucUartRxBuf); 
    ...
    //sysprintf("#### _uartBUFSpaceAlloc pucUARTFlag [%d]:0x%08x...\r\n", nNum, dev->pucUARTFlag); 
    /* initial memory */
...
INT32 uartRead(INT nNum, PUINT8 pucBuf, UINT32 uLen)
{
    ...
    {
        sysprintf(" !!! uartRead error: uLen = %d, UARTRXBUFSIZE[nNum] = %d\n", uLen, UARTRXBUFSIZE[nNum]);
        return UART_EIO;
    }
...
INT32 uartWrite(INT nNum, PUINT8 pucBuf, UINT32 uLen)
{
    ...
    if(dev->bIsUARTInitial == FALSE)
    {
        sysprintf(" !!! uartWrite error: UART_EIO [ nNum = %d]\n", nNum);
        return UART_EIO;
    }
    ...
    {
        sysprintf(" !!! uartWrite error: uLen > UARTWRITESIZE, uLen = %d\n", uLen);
        return UART_EIO;
    }
