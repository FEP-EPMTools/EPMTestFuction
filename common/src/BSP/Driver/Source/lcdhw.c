...
// by sam
static VPOST_T DEF_E50A2V1 = { 800, 480, 0, 0, 0, VPOSTB_DATA16or18, 0, VPOSTB_COLORTYPE_16M, VPOSTB_DEVICE_SYNC_HIGHCOLOR, 0x020d03a0, 0x01e00320, 0x03250321, 0x03780348, 0x01f001ed};
//static VPOST_T DEF_E50A2V1 = { 480, 272, 0, 0, 0, VPOSTB_DATA16or18, 0, VPOSTB_COLORTYPE_16M, VPOSTB_DEVICE_SYNC_HIGHCOLOR, 0x020d03a0, 0x01e00320, 0x03250321, 0x03780348, 0x01f001ed};
/* LCD build-in support list */
static VPOST_T* DisplayDevList[1] = {&DEF_E50A2V1};
...
uint8_t* vpostGetFrameBuffer(void)
{
    ...
    #ifdef _SUPPORT_FREERTOS_
    u8BufPtr = (uint8_t *)pvPortMalloc((curDisplayDev.u32DevWidth * curDisplayDev.u32DevHeight * u32BytePerPixel)+32);
    if(u8BufPtr == NULL)
    {
        sysprintf("vpostGetFrameBuffer pvPortMalloc error: %d x %d, bpp:%d, size = %d\r\n", 
                        curDisplayDev.u32DevWidth, curDisplayDev.u32DevHeight, u32BytePerPixel, (curDisplayDev.u32DevWidth * curDisplayDev.u32DevHeight * u32BytePerPixel)+32); 
    }
    else
    {
        sysprintf("vpostGetFrameBuffer pvPortMalloc ok: %d x %d, bpp:%d, size = %d\r\n", 
                        curDisplayDev.u32DevWidth, curDisplayDev.u32DevHeight, u32BytePerPixel, (curDisplayDev.u32DevWidth * curDisplayDev.u32DevHeight * u32BytePerPixel)+32); 
    }
    #else
...
uint8_t* vpostGetOSDBuffer(void)
{
    ...
    #ifdef _SUPPORT_FREERTOS_
    u8BufPtr = (uint8_t *)pvPortMalloc((curOSDDev.nOSDWidth * curOSDDev.nOSDHeight * u32BytePerPixel)+32);
    if(u8BufPtr == NULL)
    {
        sysprintf("vpostGetOSDBuffer pvPortMalloc error: %d x %d, bpp:%d, size = %d\r\n", 
                        curOSDDev.nOSDWidth, curOSDDev.nOSDHeight, u32BytePerPixel, (curOSDDev.nOSDWidth * curOSDDev.nOSDHeight * u32BytePerPixel)+32); 
    }
    else
    {
        sysprintf("vpostGetOSDBuffer pvPortMalloc OK: %d x %d, bpp:%d, size = %d\r\n", 
                        curOSDDev.nOSDWidth, curOSDDev.nOSDHeight, u32BytePerPixel, (curOSDDev.nOSDWidth * curOSDDev.nOSDHeight * u32BytePerPixel)+32); 
    }
    #else
