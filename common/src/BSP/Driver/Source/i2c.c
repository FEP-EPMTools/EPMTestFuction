...
// by sam
static SemaphoreHandle_t xi2cSemaphore[2];
/*-----------------------------------------*/
...
//add by sam
void i2cEnterCriticalSection(int32_t fd)
{
    if(i2c_device[fd].openflag == 0)
    {
        sysprintf("i2cEnterCriticalSection[%d](openflag = %d), ignore\r\n", fd, i2c_device[fd].openflag);
        return ;
    }
    xSemaphoreTake(xi2cSemaphore[fd], portMAX_DELAY);
}

void i2cExitCriticalSection(int32_t fd)
{
    if(i2c_device[fd].openflag == 0)
    {
        sysprintf("i2cExitCriticalSection[%d](openflag = %d), ignore\r\n", fd, i2c_device[fd].openflag);
        return ;
    }
    xSemaphoreGive(xi2cSemaphore[fd]);
}
...
int32_t i2cOpen(PVOID param)
{
    ...
    // by sam
    //vSemaphoreCreateBinary(xi2cSemaphore[(uint32_t)param]);
    xi2cSemaphore[(uint32_t)param] = xSemaphoreCreateMutex();
...
int32_t i2cClose(int32_t fd)
{
    ...
    //by sam
    vSemaphoreDelete(xi2cSemaphore[fd]);
    return 0;
}
