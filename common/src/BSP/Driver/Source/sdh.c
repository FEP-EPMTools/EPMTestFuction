...
int SD_Probe(unsigned int u32CardNum)
{
    ...
    {
        //sysprintf("SD_Probe FALSE!!\n");
        return FALSE;
    }
    else
    {
        //sysprintf("SD_Probe TRUE!!\n");
    }

    if (u32CardNum == SD_PORT0) 
    {
        //sysprintf("SD_Probe 1!!\n");
        if (SD_Init(&SD0) < 0)
        {
            //sysprintf("SD_Probe 1_1!!\n");
            return FALSE;
        }
        //sysprintf("SD_Probe 2!!\n");
        /* divider */
        if (SD0.CardType == SD_TYPE_MMC)
        {
            //sysprintf("SD_Probe 3!!\n");
            SD_Set_clock(20000);
        }
        else
        {
            //sysprintf("SD_Probe 4!!\n");
            SD_Set_clock(SD_FREQ);
        }
//sysprintf("SD_Probe 5!!\n");
        SD_Get_SD_info(&SD0);
//sysprintf("SD_Probe 6!!\n");
        if (SD_SelectCardType(&SD0))
        {
            //sysprintf("SD_Probe 7!!\n");
            return FALSE;
        }
//sysprintf("SD_Probe 8!!\n");
        sd0_ok = 1;
    } else if (u32CardNum == SD_PORT1) {
        if (SD_Init(&SD1) < 0)
            return FALSE;
