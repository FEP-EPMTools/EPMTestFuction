...
#define UART_2_INTERFACE_INDEX      1   //Card reader
#define UART_3_INTERFACE_INDEX      2   //Radar
#define UART_4_INTERFACE_INDEX      3   //Modem
#define UART_7_INTERFACE_INDEX      4   //Radar
#define UART_8_INTERFACE_INDEX      5   //LED
#define UART_10_INTERFACE_INDEX     6   //credit card reader
#define UART_0_INTERFACE_INDEX      7   //debug
*/
static UINT32 UARTTXBUFSIZE[UART_NUM] = {64,    64,     64,         64,             64,         64,     64,     64,             64,     64,     64      };  /* UART0~10 Tx buffer size */
static UINT32 UARTRXBUFSIZE[UART_NUM] = {1024,  64,     16*1024,    128*1024,       128*1024,    64,     64,     128*1024,       1024,     64,     16*1024 };    /* UART0~10 Rx buffer size */
...
// by sam
static SemaphoreHandle_t xuartSemaphore[UART_NUM] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
static void _uartReceiveChars(INT nNum)
{
    ...
    //add by sam
    //sysprintf(" !!! _uartReceiveChars : %d\n", nNum);
    {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        xSemaphoreGiveFromISR( xuartSemaphore[nNum], &xHigherPriorityTaskWoken );
...
static void _uartInstallISR(UINT8 ucNum)
{
    ...
    if(ucNum == UART0) {
        ...
        IRQ= UART2_IRQn;
        #if(1) //octopus reader
        dev->pvUartVector = sysInstallISR((FIQ_LEVEL_0 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart2ISR);
        #else
        ...
        IRQ= UART3_IRQn;
        //#if(1) //radar
        dev->pvUartVector = sysInstallISR((FIQ_LEVEL_0 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart3ISR);
        //#else
       ...
       // #endif
        ...
        IRQ= UART4_IRQn;
        #if(1)
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_2 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart4ISR);
        #else
        ...
        IRQ= UART7_IRQn;
        //#if(1) // radar
        dev->pvUartVector = sysInstallISR((FIQ_LEVEL_0 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart7ISR);
        //#else
...
INT uartOpen(PVOID uart)
{
    ...
    {
        // by sam
        if(xuartSemaphore[(uint32_t)dev->ucUartNo] == NULL)
            xuartSemaphore[(uint32_t)dev->ucUartNo] = xSemaphoreCreateBinary();
...
//add by sam
BaseType_t uartWaitReadEvent(INT nNum, TickType_t time)
{
    if(xSemaphoreTake(xuartSemaphore[nNum], 0) == pdTRUE)
    {
    ...
    return xSemaphoreTake(xuartSemaphore[nNum], time);
}
...
INT32 uartRead(INT nNum, PUINT8 pucBuf, UINT32 uLen)
{
    ...
    {
        sysprintf(" !!! uartRead error: uLen = %d, UARTRXBUFSIZE[%d] = %d\n", uLen, nNum, UARTRXBUFSIZE[nNum]);
        return UART_EIO;
    }


    DataLength = _uartReadRxBuf(nNum, pucBuf, uLen);
    //xSemaphoreGive(xuartSemaphore[nNum]);
    return (DataLength);
