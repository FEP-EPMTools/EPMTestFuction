...
#include "nuc970.h"
#include "sys.h"		/* Declarations of disk I/O functions */
/*--------------------------------------------------------------------------
...
)
{
	...
	for (;;) {
        ...
        {
            sysprintf("FR_INVALID_NAME : 14(%d)\n\r", di);
			return FR_INVALID_NAME;
        }
#if !_LFN_UNICODE
		w &= 0xFF;
		...
		if (!w) 
        {
            sysprintf("FR_INVALID_NAME : 16\n\r");
            return FR_INVALID_NAME;	/* Reject invalid code */
        }
#endif
		if (w < 0x80 && chk_chr("\"*:<>\?|\x7F", w)) /* Reject illegal characters for LFN */
        {
            sysprintf("FR_INVALID_NAME : 17\n\r");
			return FR_INVALID_NAME;
        }
...
)
{
    ...
    //terninalPrintf("res = %08x \r\n",&res);
    //terninalPrintf("dj = %08x \r\n",&dj);
...
)
{
	...
	fs = *fatfs;
    if(res != 0)
        sysprintf("f_getfree:  find_volume return (%d)\n", res);
	if (res == FR_OK) 
    {
...
)
{
	...
	if (res == FR_OK) {
		...
		if (_FS_RPATH && res == FR_OK && (dj.fn[NSFLAG] & NS_DOT))
        {
            sysprintf("FR_INVALID_NAME : 7\n\r");
			res = FR_INVALID_NAME;
        }
		if (res == FR_OK) {
			dir = dj.dir;
			if (!dir) {						/* Is it a root directory? */
                sysprintf("FR_INVALID_NAME : 8\n\r");
				res = FR_INVALID_NAME;
...
)
{
	...
	if (res == FR_OK) {
		...
		if (_FS_RPATH && res == FR_OK && (dj.fn[NSFLAG] & NS_DOT))
        {
            sysprintf("FR_INVALID_NAME : 10\n\r");
			res = FR_INVALID_NAME;
        }
		if (res == FR_OK) {
			dir = dj.dir;
			if (!dir) {					/* Root directory */
                sysprintf("FR_INVALID_NAME : 11\n\r");
				res = FR_INVALID_NAME;
			} else {					/* File or sub-directory */
...
int ff_cre_syncobj (BYTE vol, _SYNC_t* sobj)	/* Create a sync object */
{
    //sysprintf("INFO : ff_cre_syncobj [%d] \n\r", vol);
...
{
    //sysprintf("\r\nINFO : Lock 0x%08x\n\r", sobj);
    BaseType_t reval = xSemaphoreTake(sobj, _FS_TIMEOUT/*portMAX_DELAY*/);         
    if(reval != pdTRUE)
    {//timeout
        //sysprintf("\r\nINFO : Lock timeout\n\r");
        return 0;
