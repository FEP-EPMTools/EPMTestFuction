...
static int ehci_bulk_xfer(UTR_T *utr)
{
    ...
    //USB_debug("BULK utr=0x%x, qh=0x%x, qtd=0x%x\n", (int)utr, (int)qh, (int)qh->qtd_list);
...
static int ehci_int_xfer(UTR_T *utr)
{
    ...
    // USB_debug("ehci_int_xfer - qh: 0x%x, 0x%x, 0x%x, qtd: 0x%x\n", (int)qh, (int)qh->Chrst, (int)qh->Cap, (int)qtd);
...
static int visit_qtd(qTD_T *qtd)
{
    ...
    if ((qtd->Token & QTD_STS_ACTIVE) == 0)
    {
        ...
        {
            USB_error("qTD 0x%x error token=0x%x!  0x%x\n", (int)qtd, qtd->Token, qtd->Bptr[0]);
            if (qtd->utr->status == 0)
...
static int ehci_rh_polling(void)
{
    ...
    for (port = 0; port < EHCI_PORT_CNT; port++)
    {
        ...
        USB_debug("EHCI port%d status change: 0x%x\n", port+1, _ehci->UPSCR[port]);
        ...
        while (1)
        {
            udev = ehci_find_device_by_port(port+1);
            if (udev == NULL)
                break;
        ...
        if (connect_status == HSUSBH_UPSCR_CCS_Msk)
        {
            ...
            if (ret < 0)
            {
                USB_error("connect_device error _1! [%d]\n", ret);
                free_device(udev);
                //add by sam 2018.10.06
                break;
            }
        }
        else
