...
static int  usbh_uvc_select_alt_interface(UVC_DEV_T *vdev)
{
    ...
    for (i = 0; i < vs->num_of_alt; i++)
    {
        UVC_DBGMSG("alt %d, i=%d, best=%d, %d, %d\n", vs->alt_no[best], i, best, vs->max_pktsz[i], payload_size);
...
//terninalPrintf("vdev->img_size = %d\n\r",vdev->img_size);
    if (vdev->img_size == 0)                /* Start of a new image                       */
    {
        vs->current_frame_toggle = buff[1] & UVC_PL_FID;
        /*terninalPrintf("buff = ");
                    //terninalPrintf("data_len = %d  pkt_len = %d  buff[0] = %d\n\r",data_len,pkt_len,buff[0]);
