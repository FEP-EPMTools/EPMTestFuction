...
int uvc_parse_control_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    ...
    while (size >= sizeof(DESC_HDR_T))
    {
        ...
        case VC_INPUT_TERMINAL:
            ifd_it = (DESC_VC_IT_T *)ifd;
            UVC_P_MSG("VC Interface VC_INPUT_TERMINAL\n");
            UVC_P_MSG("    bTerminalID:    0x%x\n", ifd_it->bTerminalID);
            UVC_P_MSG("    wTerminalType:  0x%x\n", ifd_it->wTerminalType);
            UVC_P_MSG("    bAssocTerminal: 0x%x\n", ifd_it->bAssocTerminal);
            break;

        case VC_OUTPUT_TERMINAL:
            ifd_ot = (DESC_VC_OT_T *)ifd;
            UVC_P_MSG("VC Interface VC_OUTPUT_TERMINAL\n");
            UVC_P_MSG("    bTerminalID:    0x%x\n", ifd_ot->bTerminalID);
            UVC_P_MSG("    wTerminalType:  0x%x\n", ifd_ot->wTerminalType);
            UVC_P_MSG("    bAssocTerminal: 0x%x\n", ifd_ot->bAssocTerminal);
            UVC_P_MSG("    bSourceID:      0x%x\n", ifd_ot->bSourceID);
            if (ifd_ot->wTerminalType == TT_STREAMING)
            {
                UVC_P_MSG("UVC USB OT found.\n");
            }
            break;

        case VC_SELECTOR_UNIT:
            ifd_su = (DESC_VC_SU_T *)ifd;
            UVC_P_MSG("VC Interface VC_SELECTOR_UNIT\n");
            UVC_P_MSG("    bUnitID:    0x%x\n", ifd_su->bUnitID);
            UVC_P_MSG("    bNrInPins:  0x%x\n", ifd_su->bNrInPins);
        ...
        case VC_PROCESSING_UNIT:
            ...
            UVC_P_MSG("VC Interface VC_PROCESSING_UNIT\n");
            UVC_P_MSG("    bUnitID:          0x%x\n", ifd_pu->bUnitID);
            UVC_P_MSG("    wMaxMultiplier:   0x%x\n", ifd_pu->wMaxMultiplier);
            UVC_P_MSG("    bControlSize:     0x%x\n", ifd_pu->bControlSize);
            UVC_P_MSG("    bmControls:       0x%02x%02x%02x\n", ifd_pu->bmControls[2], ifd_pu->bmControls[1], ifd_pu->bmControls[0]);
            UVC_P_MSG("    bmVideoStandards: 0x%x\n", ifd_pu->bmVideoStandards);
            break;

        case VC_EXTENSION_UNIT:
            ifd_eu = (DESC_VC_EU_T *)ifd;
            UVC_P_MSG("VC Interface VC_EXTENSION_UNIT\n");
            UVC_P_MSG("    bUnitID:      0x%x\n", ifd_eu->bUnitID);
            UVC_P_MSG("    bNumControls: 0x%x\n", ifd_eu->bNumControls);
            UVC_P_MSG("    bNrInPins:    0x%x\n", ifd_eu->bNrInPins);
            break;

        case VC_ENCODING_UNIT:
            ifd_ecu = (DESC_VC_ECU_T *)ifd;
            UVC_P_MSG("VC Interface VC_ENCODING_UNIT\n");
            UVC_P_MSG("    bUnitID:   0x%x\n", ifd_ecu->bUnitID);
            UVC_P_MSG("    bSourceID: 0x%x\n", ifd_ecu->bSourceID);
            UVC_P_MSG("    iEncoding: 0x%x\n", ifd_ecu->iEncoding);
            break;
        }
#endif
...
int uvc_parse_streaming_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    ...
    while (size >= sizeof(DESC_IF_T))
    {
        ...
        case VS_INPUT_HEADER:
            UVC_P_MSG("VS Interface VS_INPUT_HEADER\n");
            UVC_P_MSG("    bNumFormats:      0x%x\n", ifd_vs_hdr->bNumFormats);
            UVC_P_MSG("    bEndpointAddress: 0x%x\n", ifd_vs_hdr->bEndpointAddress);
            UVC_P_MSG("    bTerminalLink:    0x%x\n", ifd_vs_hdr->bTerminalLink);
            break;

        case VS_FORMAT_UNCOMPRESSED:
            vsu_format = (DESC_VSU_FORMAT_T *)ifd_vs_hdr;
            UVC_P_MSG("VS Interface VS_FORMAT_UNCOMPRESSED\n");
            UVC_P_MSG("    bFormatIndex:         0x%x\n", vsu_format->bFormatIndex);
            UVC_P_MSG("    bNumFrameDescriptors: 0x%x\n", vsu_format->bNumFrameDescriptors);
            UVC_P_MSG("    guidFormat:           0x%08x%08x%08x%08x\n", vsu_format->guidFormat[0], vsu_format->guidFormat[1], vsu_format->guidFormat[2], vsu_format->guidFormat[3]);
            UVC_P_MSG("    bBitsPerPixel         0x%x\n", vsu_format->bBitsPerPixel);
            UVC_P_MSG("    bDefaultFrameIndex:   0x%x\n", vsu_format->bDefaultFrameIndex);
            UVC_P_MSG("    bAspectRatioX:        0x%x\n", vsu_format->bAspectRatioX);
            UVC_P_MSG("    bAspectRatioY:        0x%x\n", vsu_format->bAspectRatioY);
            ...
            for (i = 0; (i < vsu_format->bNumFrameDescriptors) && (vc->num_of_frames < UVC_MAX_FRAME); i++)
            {
                ...
                UVC_P_MSG("VS Interface VS_FRAME_UNCOMPRESSED\n");
                UVC_P_MSG("    bFormatIndex: 0x%x\n", vsu_frame->bFrameIndex);
                UVC_P_MSG("    wWidth:       0x%x\n", vsu_frame->wWidth);
                UVC_P_MSG("    wHeight       0x%x\n", vsu_frame->wHeight);
                UVC_P_MSG("    dwMinBitRate: 0x%x\n", vsu_frame->dwMinBitRate);
                UVC_P_MSG("    dwMaxBitRate: 0x%x\n", vsu_frame->dwMaxBitRate);
                UVC_P_MSG("    dwDefaultFrameInterval:  0x%x\n", vsu_frame->dwDefaultFrameInterval);
        ...
        case VS_FORMAT_MJPEG:
            ...
            UVC_P_MSG("VS Interface VS_FORMAT_MJPEG\n");
            ...
            for (i = 0; (i < mjpg_format->bNumFrameDescriptors) && (vc->num_of_frames < UVC_MAX_FRAME); i++)
            {
                ...
                UVC_P_MSG("VS Interface VS_FRAME_MJPEG\n");
    ...
    UVC_P_MSG("\n\n----------------------------------------------------------\n");
    UVC_P_MSG("[Video Streaming interface parsing result dump]\n");
    for (i = 0; i < vs->num_of_alt; i++)
    {
        UVC_P_MSG("  Alt %d, wMaxPacketSize = %d\n", vs->alt_no[i], vs->max_pktsz[i]);
    }
    return 0;
}
