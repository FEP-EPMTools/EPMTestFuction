...
/* XY PARA */
#define OFFSET_X 80
#define RADAR_0_MSG_X 50  //180
#define RADAR_0_MSG_Y 150
#define RADAR_1_MSG_X 550 //580
#define RADAR_1_MSG_Y 150

#define X_HEAD_TITLE    50 //90
...
static void updateBG(void)
{
    ...
    if(param3==0){
        ...
        EPDDrawString(FALSE,"Radar Tool",X_HEAD_TITLE,Y_HEAD_TITLE);
        
        EPDDrawString(FALSE,"Status:\nRadar 1",RADAR_0_MSG_X,RADAR_0_MSG_Y);
        EPDDrawString(FALSE,"\nRadar 2",RADAR_1_MSG_X,RADAR_1_MSG_Y);
        EPDDrawString(TRUE,"Press } To Exit",BTM_DISCRIPT_BAR_X,BTM_DISCRIPT_BAR_Y);
    }
    else if(param3==POWER_SET_MODE){
        EPDDrawMulti(TRUE,EPD_PICT_ALL_WHITE_INDEX,0,0);
        EPDDrawString(FALSE,"Radar Power Set",X_HEAD_TITLE,Y_HEAD_TITLE);
        

        EPDDrawString(FALSE,"Power:",SET_POWER_TITLE_X,SET_POWER_TITLE_Y);
        EPDDrawString(FALSE,"Radar 1",RADAR_0_TITLE_X,RADAR_0_TITLE_Y);
        EPDDrawString(FALSE,"Radar 2",RADAR_1_TITLE_X,RADAR_1_TITLE_Y);
...
/* Exported Functions          */
/*-----------------------------------------*/
BOOL GuiRadarOnDraw(uint8_t oriGuiId, uint8_t para, int para2, int para3)
{   
    param3=para3;
...
BOOL GuiRadarKeyCallback(uint8_t keyId, uint8_t downUp)
{   
    BOOL reVal = FALSE; 
    ...
    if(GUI_KEY_DOWN_INDEX == downUp)
    {
        ...
        switch(keyId)
        {
            case GUI_KEYPAD_ONE:
                ...
                {
                    oldIndex=nowIndex;
                    //EPDDrawMulti(FALSE,EPD_PICT_LINE_SMALL_2_INDEX,cursorPos[nowIndex].x,cursorPos[nowIndex].y);
                    nowIndex--;
                    //EPDDrawMulti(TRUE,EPD_PICT_LINE_SMALL_2_I_INDEX,cursorPos[nowIndex].x,cursorPos[nowIndex].y);
                    reVal = TRUE;
                }
...
BOOL GuiRadarTimerCallback(uint8_t timerIndex)
{
...
BOOL GuiRadarPowerCallbackFunc(uint8_t type, int flag)
{
