...
static void updateBG(void)
{
    ...
    EPDDrawStringMax(FALSE,"Calibration",90,28,FALSE);
...
/* Exported Functions                      */
/*-----------------------------------------*/
BOOL GuiCalibrationOnDraw(uint8_t oriGuiId, uint8_t reFreshPara, int item, int keyinCallbackFun)
{
...
BOOL GuiCalibrationKeyCallback(uint8_t keyId, uint8_t downUp)
{
    ...
    if(keyIgnoreFlag)
    {
        //terninalPrintf(" [INFO GUI] <Calibration> Key:  ignore...\n"); 
        return reVal;
    }
    if(keyinCallback == NULL)
    {
        terninalPrintf(" [INFO GUI] <Calibration> KeyCallback:  Function Pointer ERROR!!!\n"); 
        return reVal;
    }
    if(GUI_KEY_DOWN_INDEX == downUp)
    {
        switch(keyId)
        {
            case GUI_KEYPAD_ONE:
                if(!GetTesterFlag())
                {
                    //EPDDrawString(TRUE,"           ",300+400,100+(44*(nowIndex+2)));
        ...
        case UPDATE_DATA_TIMER:
            if(oldIndex!=nowIndex)
            {
                if(cleanMsgAfterMove)
                {
                    ...
                    //EPDDrawString(TRUE,"           ",300+400,100+(44*(oldIndex+2)));
                }
