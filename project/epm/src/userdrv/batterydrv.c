...
#define SVOLTAGE_DIVIDER_1              685  //750     // CPU internal divider resistor (7.5K)
...
static BOOL CADtenConnectFlag = FALSE;

static uint32_t ajustSolarBatFactor = 1000;

static BOOL Bat1INTFlag     = FALSE;
static BOOL Bat2INTFlag     = FALSE;
static BOOL SolarBatINTFlag = FALSE;
...
static void measureBattery()
{
    ...
    for(ainIndex = AIN_START_INDEX; ainIndex < AIN_START_INDEX + AIN_NUM; ainIndex++)
    {
        ...
        //UINT32 reg = inpw(REG_ADC_CONF);
        //terninalPrintf("REG_ADC_CONF = 0x%08x\r\n", reg);
...
//            adcIoctl(VBPOWER_OFF,(UINT32)0,0);
//        }
//        else {
//            adcIoctl(NAC_OFF,0,ainIndex);
//        }
    }
...
INT32 EINT5Callback(UINT32 status, UINT32 userData)
{
    ...
    {
        /*
        if(GPIO_ReadBit(BATTERY_2_LOW_DETECT_PORT, BATTERY_2_LOW_DETECT_PIN) == 0)
        {
...
{   
    ...
    GPIO_OpenBit(BATTERY_1_ENABLE_PORT, BATTERY_1_ENABLE_PIN, DIR_OUTPUT, PULL_UP);  
    GPIO_OpenBit(BATTERY_2_ENABLE_PORT, BATTERY_2_ENABLE_PIN, DIR_OUTPUT, PULL_UP);
    ...
    GPIO_ClrBit(BATTERY_SOLAR_ENABLE_PORT, BATTERY_SOLAR_ENABLE_PIN);
    #endif   
    ...
    if(readMBtestFunc())
        //GPIO_EnableTriggerType(GPIOH, BATTERY_2_LOW_DETECT_PIN, RISING);
        GPIO_EnableTriggerType(GPIOH, BATTERY_2_LOW_DETECT_PIN, FALLING);
    else
    ...
    /* Confingure PH5 to both-edge trigger */
    if(readMBtestFunc())
        //GPIO_EnableTriggerType(GPIOH, BATTERY_1_LOW_DETECT_PIN, RISING);
        GPIO_EnableTriggerType(GPIOH, BATTERY_1_LOW_DETECT_PIN, FALLING);
    else
...
UINT32 SolarBatteryGetValue(void)
{
    return (SREAL_VOLTAGE(voltageValue[BATTERY_0_BATTERY_INDEX]));
}
