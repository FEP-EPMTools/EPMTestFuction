...
#if (ENABLE_BURNIN_TESTER)
static uint32_t nt066eBurninCounter = 0;
static uint32_t nt066eBurninErrorCounter = 0;
#endif
...
#if (ENABLE_BURNIN_TESTER)
static void vNT066ETestTask(void *pvparamOuteters)
{
    time_t lastTime = GetCurrentUTCTime();
    time_t currentTime;
    BOOL testLoop = FALSE;
    BOOL status;
    terninalPrintf("vNT066ETestTask Going...\r\n");
    while (TRUE)
    {
        if (GetPrepareStopBurninFlag())
        {
            terninalPrintf("vNT066ETestTask Terminated !!\r\n");
            vTaskDelete(NULL);
        }
        currentTime = GetCurrentUTCTime();
        if ((currentTime - lastTime) > BURNIN_NT066E_INTERVAL)
        {
            //terninalPrintf("vNT066ETestTask heartbeat.\r\n");
        ...
        testLoop = FALSE;
        status = NT066ESetTriggerLevel(10);
        nt066eBurninCounter++;
        if (!status) {
            nt066eBurninErrorCounter++;
        }
        //terninalPrintf("vNT066ETestTask, status=%d\r\n", status);
    }
}
#endif
...
#if (ENABLE_BURNIN_TESTER)
    else if (EnabledBurninTestMode())
    {
        xTaskCreate(vNT066ETestTask, "vNT066ETestTask", 1024*10, NULL, NT066E_DRV_THREAD_PROI, NULL);
    }
#endif
...
BOOL NT066EResetChip(void)
{
    ...
    if(NT066ESetTriggerLevel(10) == FALSE)
    {
        return FALSE;
    } 
...
{
    return nt066eBurninCounter;
}

uint32_t GetNT066EBurninTestErrorCounter(void)
{
    return nt066eBurninErrorCounter;
}
#endif
