...
static CmdRegAction ftpDeleteFileCmdRegAction[MODEM_STAGE_CMD_REG_ACTION_NUM] =    {{CMD_REQ_OK, TRUE}, 
                                                                                    {CMD_REQ_CME_ERROR, FALSE}, 
                                                                                    {CMD_REQ_FTP_DISCONNECT, TRUE}, 
                                                                                    {CMD_REQ_NULL, FALSE}};
static ModemCmdItem ftpDeleteFileCmdItem = {(uint8_t*)NULL,    ftpDeleteFileCmdRegAction, 500, 5000};
...
unsigned char readBuff[128*1024]; //readBuff[64*1024];  //readBuff[4*1024];  
...
static char DeleteFileName[100];
...
static ModemReturnValue actionCmdPureFTP(ModemCmdItem* pModemCmdItemTmp, ParserType parserType,char* rbuff,int rlen,int* retunlen,UINT32 readtime) 
{
    BOOL reVal =FALSE;
    CmdActionValue actionValue;
    CmdReq cmdReq = CMD_REQ_NULL;
...
BOOL FtpClientSendFile(BOOL needChdirFlag, char* preDirName, char* dirName, char* fileName, uint8_t* buff, int len)
{
    ...
    else
    {
        ...
        char numStr[10];
        int rnum;
        //terninalPrintf("FTP send datasize = %d\r\n",len);  
        if (len > 0)
        {
            //modemReval = actionCmdPure(&ftpSendDataCmdItem, PARSER_TYPE_NORMAL);
            modemReval = actionCmdPurePro(&ftpSendDataCmdItem, PARSER_TYPE_NORMAL,rbuff,sizeof(rbuff),&retnlen, 100);
            if(modemReval == MODEM_RETURN_ERROR)
            {
            ...
            else
            {
                /*
                terninalPrintf("rbuff = ");
                for(int m=0;m<retnlen;m++)
                    terninalPrintf("%02x ",rbuff[m]);
                terninalPrintf("\r\n");
                */
                for(int i=0;i<retnlen;i++)
                {
                    if(rbuff[i] == ',')
                    {
                        rindex = i;
                        for(int j=(rindex+1);j<retnlen;j++)
                        {
                            if((rbuff[j] > 0x39) || (rbuff[j] < 0x30) || (j == (retnlen-1)))
                            {
                                rindexend = j;
                                //terninalPrintf("rindex = %d\r\n",rindex);
                                //terninalPrintf("rindexend = %d\r\n",rindexend);
                                memcpy(numStr,(char*)rbuff + rindex + 1,rindexend - rindex -1);
                                /*
                                terninalPrintf("FTP feedback numStr = ");
                                for(int k=0;k<(rindexend - rindex -1);k++)
                                    terninalPrintf("%02x ",numStr[k]);
                                terninalPrintf("\r\n");
                                */
                                rnum = atoi(numStr);
                                //terninalPrintf("FTP feedback datasize = %d\r\n",rnum);
                                //break;
                                if(rnum == len)
                                    return TRUE;
                                else
                                {
                                    //terninalPrintf("EPM send datasize = %d\r\n",len);
                                    //terninalPrintf("FTP feedback datasize = %d  ERROR\r\n",rnum);
                                    return FALSE;
                                }
                            } 
                        }
                    ...
                    {
                        //terninalPrintf("FTP feedback cmd ERROR\r\n");
                        return FALSE;
                    }
                }
...
BOOL FtpClientSendFileEx(BOOL needChdirFlag, char* preDirName, char* dirName, char* fileName, uint8_t* buff, int len,
                       int* err1,int* err2)
{
    BOOL needMkdirFlag = FALSE;
    //BOOL needChdirFlag = TRUE;
    ...
    */
    ...
    else
    {
        ...
        {
            //modemReval = actionCmdPure(&ftpSendDataCmdItem, PARSER_TYPE_NORMAL);
            modemReval = actionCmdPureFTP(&ftpSendDataCmdItem, PARSER_TYPE_NORMAL,rbuff,sizeof(rbuff),&retnlen, 100);
            if(modemReval == MODEM_RETURN_ERROR)
            {
                sysprintf(" == Send Ftp Client Data Error\r\n");
                //QModemAbortDataMode();
...
BOOL FtpClientDeleteFile(BOOL needChdirFlag, char* preDirName, char* dirName, char* fileName)
{
    BOOL needMkdirFlag = FALSE;
    //BOOL needChdirFlag = TRUE;
    uint8_t cmdFtpSendInit[256];
...
BOOL FtpClientGetFileLite(char* dirName, char* fileName)
{
    uint8_t cmdFtpGetInit[256];
...
#define WEBPOST_UART_WRITE_SIZE     256
//AT+QFTPSIZE=?est_my1.txt?//Query size of ?est_my1.txt?on FTP server.
//OK
//+QFTPSIZE: 1000
static CmdRegAction ftpGetSizeCmdRegAction[MODEM_STAGE_CMD_REG_ACTION_NUM] =    {{CMD_REQ_FTP_GET_SIZE_OK, TRUE}, 
                                                                                    {CMD_REQ_CME_ERROR, FALSE}, 
                                                                                    {CMD_REQ_NULL, FALSE}, 
                                                                                    {CMD_REQ_NULL, FALSE}};
static ModemCmdItem ftpGetSizeCmdItem = {(uint8_t*)NULL,    ftpGetSizeCmdRegAction, 100, 5000};

static ModemReturnValue actionCmdPure2(ModemCmdItem* pModemCmdItemTmp, ParserType parserType, int atLeastSize) 
{
    BOOL reVal =FALSE;
    CmdActionValue actionValue;
    CmdReq cmdReq = CMD_REQ_NULL;
...
//        int i = 0;
        ...
        int leftLen = 0;
        int n;
        buff = pModemCmdItemTmp->cmd; 
        len = strlen((char*) pModemCmdItemTmp->cmd);
        for(int i = 0; i< len/WEBPOST_UART_WRITE_SIZE; i++)
        {
            n = QModemWrite(buff + i*WEBPOST_UART_WRITE_SIZE, WEBPOST_UART_WRITE_SIZE);
            if(n != WEBPOST_UART_WRITE_SIZE)
            {
                sysprintf(" == WEBPOST Client write error 1\r\n");
                continueFlag = FALSE;
                break;
            }
            else
        ...
        if(continueFlag)
        {
            leftLen = len-(len/WEBPOST_UART_WRITE_SIZE*WEBPOST_UART_WRITE_SIZE);
            if(leftLen > 0)
            {
                n = QModemWrite(buff + len/WEBPOST_UART_WRITE_SIZE*WEBPOST_UART_WRITE_SIZE, leftLen);
                if(n != leftLen)
                {
                    sysprintf(" == WEBPOST Client write error 2\r\n");
                    //break;
                }
                else
                {
    ...
    while (waitTimes > 0) 
    {
        ...
        {
            //ATCmdSetReceiveDebugFlag(TRUE);
            cmdReq = atCmdProcessReadData2(readBuff, n, parserType, atLeastSize);
            //ATCmdSetReceiveDebugFlag(FALSE);
            sysprintf("\r\n --TEST---  !!! get cmeReq = %d !!!\n", cmdReq);
            if (cmdReq != CMD_REQ_NULL) 
            {
    ...
    #if(SHOW_ACTION_COMMAND_INFO)
    if(strlen((char*) pModemCmdItemTmp->cmd) < 128)
    {
    ...
    if(waitTimes == 0)
    {
        ...
        {
            char str[1024];
            if(strlen((char*)pModemCmdItemTmp->cmd) < (sizeof(str) - strlen("\r\n [---- ERROR ----] - actionCmdPure ERROR- timeout (cmd: %s)\r\n")) - 10)
            {
                sprintf(str, "\r\n [---- ERROR ----] - actionCmdPure ERROR- timeout (cmd: %s)\r\n", pModemCmdItemTmp->cmd);
            }
            else
...
static char* parserString2(char* str, uint32_t strRealLen, char* startStr, char* endStr, uint32_t* destLen, int exitType)
{
    int i;
    char* startIndex = NULL;
    *destLen = 0;
    sysprintf("parserString : strRealLen = %d, startStr(len=%d) = [%s], endStr(len=%d) = [%s]\r\n", strRealLen, strlen(startStr), startStr, strlen(endStr), endStr);
    if(strRealLen == 0)
        strRealLen = strlen(str);
    
    for (i = 0; i < (strRealLen - strlen(endStr))+1; i++)
    {
        //sysprintf("parserString [%d]: CHECK\r\n", i);
        if(startIndex == 0)
        {
            if(memcmp(str+i, startStr, strlen(startStr)) == 0)
            {
                startIndex = str + i + strlen(startStr);
                sysprintf("parserString [%d]: GET START\r\n", i);
                i = i + strlen(startStr);
            }
        }
        else
        {
            ...
            {
                *destLen = str + i - startIndex;
                sysprintf("parserString [%d]: GET END\r\n", i);
                if(exitType == 1)
                {
                    break;
                }
...
BOOL FtpClientGetFilePure(char* dirName, char* fileName, uint8_t** dataBuff, uint32_t* dataLen, uint32_t waitTime)
{
    uint8_t cmdFtpGetInit[512];
    
    terninalPrintf(" == FtpClientGetFilePure [%s][%s]\r\n", dirName, fileName);

    sprintf((char*)cmdFtpGetInit, "AT+QFTPCWD=\"/%s\"\r\n", dirName);  
  
    ftpChDirCmdItem.cmd = cmdFtpGetInit;
    ModemReturnValue modemReval = actionCmdPure2(&ftpChDirCmdItem, PARSER_TYPE_NORMAL, 0);
    if(modemReval == MODEM_RETURN_ERROR)
    {
        //sysprintf("\r\n -- timeout (%d) -- *stageIndex = %d \r\n", pModemStageItemTmp->waitTime, *stageIndex);
        return FALSE;
    ...
    {
        sprintf((char*)cmdFtpGetInit, "AT+QFTPSIZE=\"%s\"\r\n", fileName);
        sysprintf(" == FtpClientGetFilePure change dir OK, we send [%s]\r\n", cmdFtpGetInit);  
        ftpGetSizeCmdItem.cmd = cmdFtpGetInit;
        ModemReturnValue modemReval = actionCmdPure2(&ftpGetSizeCmdItem, PARSER_TYPE_NORMAL, 0);
        if(modemReval == MODEM_RETURN_ERROR)
        {
            //sysprintf("\r\n -- timeout (%d) -- *stageIndex = %d \r\n", pModemStageItemTmp->waitTime, *stageIndex);
            //QModemAbortDataMode();
        ...
        else
        {
            uint32_t destLen;
            int fileSize;
            sysprintf(" == FtpClientGetFilePure get size OK [%s]\r\n", ATCmdDataTempBuffer(NULL));                         
            char* str = parserString2((char*)ATCmdDataTempBuffer(NULL), 0, "+QFTPSIZE: 0,", "\r\n", &destLen, 0);                        
            if(str != NULL)
            {
                str[destLen] = 0x0;
                fileSize = atoi (str);
                //sysprintf(" == Get File size: [%s] [%d]\r\n", str, fileSize); 
                terninalPrintf(" == FtpClientGetFilePure [%s][%s] (len = %d) Start download...\r\n", dirName, fileName, fileSize);
                //AT+QFTPGET=\"%s\", "COM:"        
                sprintf((char*)cmdFtpGetInit, "AT+QFTPGET=\"%s\", \"COM:\"\r\n", fileName);
                sysprintf(" == get size OK, we send [%s]\r\n", cmdFtpGetInit);  
                ftpGetDataCmdItem.cmd = cmdFtpGetInit;
                ftpGetDataCmdItem.waitTime = waitTime;
                ModemReturnValue modemReval = actionCmdPure2(&ftpGetDataCmdItem, PARSER_TYPE_FTP, fileSize);
                if(modemReval == MODEM_RETURN_ERROR)
                {
                    //sysprintf("\r\n -- timeout (%d) -- *stageIndex = %d \r\n", pModemStageItemTmp->waitTime, *stageIndex);
                    QModemAbortDataMode();
                ...
                else
                {
                    //sysprintf(" == Web Get FTP contain OK [%s]\r\n", ATCmdDataTempBuffer(NULL));
                    uint32_t destLen;
                    char* atBuff = (char*)ATCmdDataTempBuffer(&destLen);
                    sysprintf(" == Web Get FTP contain OK [ATCmdDataTempBuffer len = %d, strlen = %d]\r\n", destLen, strlen(atBuff));
                    char* str = parserString2(atBuff, destLen, "\r\nCONNECT\r\n", "\r\nOK\r\n", &destLen, 0);
                    sysprintf(" == Web Get FTP contain OK [parserString len = %d]\r\n", destLen);
                    *dataLen = destLen;
                    *dataBuff = (uint8_t*)str;
                    return TRUE;
                }              
            }
            else
...
BOOL FtpDeleteFile(char* filename)
{
    //memcpy(DeleteFileName,filename,sizeof(DeleteFileName));
    sprintf((char*)DeleteFileName,"%s%s%s",FTP_DEL_FIL_FRONT,filename,FTP_DEL_FIL_END);
    ftpDeleteFileCmdItem.cmd = DeleteFileName;
    if(actionCmdPure(&ftpDeleteFileCmdItem, PARSER_TYPE_NORMAL) == MODEM_RETURN_OK)
    {
        sysprintf(" == FtpDeleteFile OK\r\n"); 
        return TRUE;
    }
    else
    {
        sysprintf(" == FtpDeleteFile ERROR\r\n"); 
        return FALSE;
    }
}
