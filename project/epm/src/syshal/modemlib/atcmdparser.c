...
static uint32_t dataTempBufferLen;

static uint8_t dataTempBuffer2[64*1024];
static uint32_t dataTempBufferLen2;
...
static CmdItem cmdList[] = { { CMD_REQ_EXIST_OK, (uint8_t*)"\r\n+CPIN: READY\r\n", WAIT_OK_FLAG_TRUE},     
                                ...
                                {CMD_REQ_FTP_GET_SIZE_OK, (uint8_t*)"\r\n+QFTPSIZE: ", WAIT_OK_FLAG_FALSE},
...
static uint8_t cmdDataTmp[1024*1024];
static int cmdDataTmpIndex = 0;;
static uint8_t cmdData[1024*1024];
...
CmdReq atCmdProcessReadData2(uint8_t* data, int len, ParserType parserType, int atLeastSize)
{
    //printfBuffData((char*)"DATA TEMP", cmdDataTmp, cmdDataTmpIndex);
    ...
    switch(parserType)
    {
        ...
        case PARSER_TYPE_FTP:
            ...
            break;
        /*
        case PARSER_TYPE_SOCKET:
            pTargetCmdList = &socketCmdList;
            break;
        case PARSER_TYPE_SMS:
            pTargetCmdList = &smsCmdList;
            break;
        case PARSER_TYPE_FTP_GET_LIST:
            pTargetCmdList = &ftpGetListCmdList;
            break;
        case PARSER_TYPE_SPECIAL:
            pTargetCmdList = &targetSpecialCmdList;
            break;
        */
    }
...
//#endif
    ...
    if(atLeastSize < totalCmdDataLen)
    {
        while (dataCurrentIndex < totalCmdDataLen)
        {   
        redo:
            ...
            if(targetHeaderIndex != -1)
            {
                ...
                if(targetStartTempPr)
                {
                    //targetEndTempPr = findStartAddress(targetStartTempPr+1, dataLeftLen - (targetStartTempPr - dataCurrentPr), pTargetCmdList->item[i]->endStr, strlen((char*)pTargetCmdList->item[i]->endStr));
                    //限制最少長度是 atLeastSize
                    uint8_t* targetStartTempPr2 = targetStartTempPr+atLeastSize;
                    targetEndTempPr = findStartAddress(targetStartTempPr2+1, dataLeftLen - (targetStartTempPr2 - dataCurrentPr), pTargetCmdList->item[i]->endStr, strlen((char*)pTargetCmdList->item[i]->endStr));
                    #if(ENABLE_AT_CMD_DEBUG_2)
                    sysprintf("   =find end=> %d\r\n", targetEndTempPr - dataCurrentPr);
                    #endif
                    if(targetEndTempPr)
    ...
    else
    {
        ...
        printfBuffData((char*)"Copy to TEMP", dataCurrentPr, dataLeftLen);
        #endif
        memcpy(cmdDataTmp, cmdData, totalCmdDataLen);
        cmdDataTmpIndex = totalCmdDataLen;
    }
    return reVal;
}
...
uint8_t* ATCmdDataTempBuffer(uint32_t* len)
{
    if(len != NULL)
    {
...
uint8_t* ATCmdDataTempBuffer2(uint32_t* len)
{
    if(len != NULL)
    {
