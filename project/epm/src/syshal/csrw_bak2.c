...
//send last dummy packet to trigger RW purge UD
            ...
            else
            {
                ...
                {
                    errorCounter++;
                    printf ("Fail [g_len == 0] (errorCounter = %d) in getting init recv response rspCode:%x, rspErr:%x\n", errorCounter, contRecvRsp.f_rspCode, contRecvRsp.f_err);
                    if(errorCounter < XFILE_RETRY_TIMES)                        
                        break;
                    else
                        return;
                }
    ...
    if(FileAgentAddData(FILE_AGENT_STORAGE_TYPE_AUTO, "1:", outFile, xFileDataPr, xFileDataLen, FILE_AGENT_ADD_DATA_TYPE_OVERWRITE, FALSE, TRUE, TRUE) ==  FILE_AGENT_RETURN_ERROR ) 
    {
        terninalPrintf ("CmdXFileContRecv: Save EEPROM FAIL, outFile fname:%s (xFileDataLen = %d)\n", outFile, xFileDataLen);
    }
    else
    {
        terninalPrintf ("CmdXFileContRecv: Save EEPROM SUCCESS, outFile fname:%s (xFileDataLen = %d)\n", outFile, xFileDataLen);
    }
    ...
    {
        terninalPrintf ("CmdXFileContRecv: Save SD FAIL, outFile fname:%s (xFileDataLen = %d)\n", outFile, xFileDataLen);
    }
    else
    {
        terninalPrintf ("CmdXFileContRecv: Save SD SUCCESS, outFile fname:%s (xFileDataLen = %d)\n", outFile, xFileDataLen);
    }
