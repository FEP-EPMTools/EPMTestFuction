...
/*-----------------------------------------*/
#define TS2000_DRV_UART         UART_2_INTERFACE_INDEX
#define UART_BARDRATE           115200 //ts2000
//#define UART_BARDRATE           57600 //ts1000
#define CAD_UART_RX_BUFF_LEN    128 //need check uart.c UARTRXBUFSIZE[UART_NUM] 
...
#define CARD_MESSAGE_TYPE_TURN_ON_RF        0x05
#define CARD_MESSAGE_TYPE_TURN_OFF_RF       0x06
...
static UINT8 read_ivn[] =   {0xEA,01,0x00,0x00,0x01,0x00,0x90,0x00};//ts2000
//static UINT8 read_ivn[] =  {0xEA,0x05,0x00,0x00,0x01,0x00,0x90,0x00};//ts1000
...
//static UINT8 turnon_rf[] =  {0xEA,0x01,0x25,0x00,0x01,0x00,0x90,0x00};
//static UINT8 turnoff_rf[] =  {0xEA,0x01,0x26,0x00,0x01,0x00,0x90,0x00};
...
static BOOL useCallBackFlag = FALSE;
static BOOL callBackFlag;
static uint16_t callBackParaValue;
/*-----------------------------------------*/
/* prototypes of static functions          */
/*-----------------------------------------*/
static void setcallBackValue(BOOL flag, uint16_t paraValue)
{
    useCallBackFlag = TRUE;
    callBackFlag = flag;
    callBackParaValue = paraValue;
}
...
static int uart_r(uint8_t msgType)
{
    int timeoutCounter;
    ...
    switch(msgType)
    {
        ...
        case CARD_MESSAGE_TYPE_IPASS_EXECUTE:
            timeoutCounter = 20;
            break;
        case CARD_MESSAGE_TYPE_TURN_ON_RF:
            timeoutCounter = 20;
            break;
        case CARD_MESSAGE_TYPE_TURN_OFF_RF:
            timeoutCounter = 20;
            break;        
        default:
            timeoutCounter = 20;
    ...
    while(1)
    {
        ...
        else
        {
            if(rx_count>=5)
            {
                //sysprintf("\r\n !!!  uart_r break section 1: %d ticks, timeoutCounter = %d\n", xTaskGetTickCount() - mTick, timeoutCounter);
                break;
            }
            ...
            {              
                rx_bcount = 0;
                //sysprintf("uart_r timeout 1, break (%d ticks)....\n", xTaskGetTickCount() - mTick);
                goto exitHandle;
            }
            //sysprintf(".");
    ...
    while(len>rx_bcount)
    {
        ...
        else
        {
            if(timeoutCounter-- == 0)
            {
                ...
                sysprintf("uart_r timeout 2, break (%d ticks)....\n", xTaskGetTickCount() - mTick);
...
exitHandle:
    ...
    if(rx_bcount!=0)
        sysprintf("uart_r OK case %d ticks....\n", xTaskGetTickCount() - mTick);
    #endif
...
{    
    ...
    switch(msgType)
    {
        ...
        case CARD_MESSAGE_TYPE_TIME:  
            ...
            //沒有LRC
            //sysprintf("=> data_time(%d): year %d, Month %d, Day %d, Hour %d, Minute %d, Second %d\n", 
            //                rx_buff[5], rx_buff[6], rx_buff[7], rx_buff[8], rx_buff[9], rx_buff[10], rx_buff[11]);
            
            extern UINT32 sysDOS_Time_To_UTC(DateTime_T ltime);//sys_timer.c
            unix_Time.year  = rx_buff[6] + 2000;
            unix_Time.mon   = rx_buff[7];
            unix_Time.day   = rx_buff[8];
            unix_Time.hour  = rx_buff[9];
            unix_Time.min   = rx_buff[10];
        ...
        case CARD_MESSAGE_TYPE_ICD:  
            ...
            {                
                ...
                if(dataLen == 1)
                {
                    ...
                    //CardSetPollingTime(portMAX_DELAY);
                    setcallBackValue(FALSE, rx_buff[5]);
                    goto exitFuncion;
                }
                else if(dataLen == 2)
                {
                    ...
                    //CardSetPollingTime(portMAX_DELAY);
                    setcallBackValue(FALSE, errorcode);
                    goto exitFuncion;
                }
...
static BOOL Read_Time(void)
{
    ...
    */
    ...
    {
        //vTaskDelay(10/portTICK_RATE_MS); 
        count=uart_r(CARD_MESSAGE_TYPE_TIME);
...
{ 
    ...
    if(flag)
    {
        nret = uart_w(turnon_rf,sizeof(turnon_rf));
        targetLen = sizeof(turnon_rf);
        targetCmdId = CARD_MESSAGE_TYPE_TURN_ON_RF;
    }
    else
    {
        nret = uart_w(turnoff_rf,sizeof(turnoff_rf));
        targetLen = sizeof(turnoff_rf);
        targetCmdId = CARD_MESSAGE_TYPE_TURN_OFF_RF;
    }
    if(nret != targetLen)
    {
        sysprintf("Turn_RF(%d) uart_w size error [%d: %d]\n", flag, nret, sizeof(read_cn));
    }
    else
    {
        //vTaskDelay(10/portTICK_RATE_MS);     
        count=uart_r(targetCmdId);
        ...
        {
           reVal = parserMessage(targetCmdId, rx_buff, count);
        }
    }
...
{ 
    ...
    {
        //vTaskDelay(10/portTICK_RATE_MS);     
        count=uart_r(CARD_MESSAGE_TYPE_CN);
        //sysprintf("we get %d char! @ Read_CN\n",count);
        if(count == 0)
        {        
        }
...
static BOOL Read_IVN(void)
{
    ...
    else
    {
        ...
        if(count != 0)
        {
            memcpy(verStr, rx_buff+5, rx_buff[3]<<8|rx_buff[4]);
            sysprintf(" => IVN len = %d:[%s]\n", rx_buff[3]<<8|rx_buff[4], verStr);
            return TRUE;
        }
    }
    return FALSE;
...
<--- ICD ---
    ...
    */
    ...
    {
        //vTaskDelay(10/portTICK_RATE_MS); 
        count=uart_r(CARD_MESSAGE_TYPE_ICD);
...
static BOOL iPass_Execute(char CmdType, uint16_t deduct)
{
    ...
    deduct_idata[28]=0xa1;//交易系統編號
    deduct_idata[29]=0x2d;//交易停車場地點代碼
    deduct_idata[30]=0x00;//停車場代碼 x2
    deduct_idata[31]=0x00;
    deduct_idata[32]=data_mn[0];//交易機器編號
    ...
    {
        //vTaskDelay(10/portTICK_RATE_MS); 
        count=uart_r(CARD_MESSAGE_TYPE_IPASS_EXECUTE);
...
{
    sysprintf("TS2000DrvInit!!\n");
    pUartInterface = UartGetInterface(TS2000_DRV_UART);
    if(pUartInterface == NULL)
    {
        sysprintf("TS2000DrvInit ERROR (pUartInterface == NULL)!!\n");
        return FALSE;
    }
    if(pUartInterface->initFunc(UART_BARDRATE) == FALSE)
    {
        sysprintf("TS2000DrvInit ERROR (initFunc false)!!\n");
        return FALSE;
    }

    sysprintf("TS2000DrvInit OK!!\n");
    return TRUE;
}
...
uint8_t TS2000CheckReader(void)
{
    ...
    sysprintf(" !!! TS2000CheckReader enter  !!!...\r\n");
    ...
    {//因為break而退出
        ...
        sysprintf("\r\n !!! TS2000CheckReader break [%d] ticeks!!!...\r\n",  xTaskGetTickCount() - powerUpTick);        
    }
    else
    {
        if(reVal == TSREADER_CHECK_READER_ERROR)
        {
            sysprintf("\r\n !!! TS2000CheckReader ERROR [%d] ticeks!!!...\r\n",  xTaskGetTickCount() - powerUpTick);

        }
        else
        {
            sysprintf("\r\n !!! TS2000CheckReader OK [%d] ticeks, times = %d!!!...\r\n",  xTaskGetTickCount() - powerUpTick, times);
        }
    }

    //if(Turn_RF(TRUE) == FALSE)
    //{
    //    sysprintf("\r\n !!! TS2000CheckReader return Turn_RF(TRUE) fail\n");
    //    return FALSE;  
    //}
    return reVal;
}
...
{   
    ...
    if(retval)
        sysprintf(" [INFO Card Reader] <TS2000Process>  : [%d] ticks\n", xTaskGetTickCount() - tickLocalStart);   


    if(useCallBackFlag)
    {
        if(ptsreaderDepositResultCallback != NULL)
            ptsreaderDepositResultCallback(callBackFlag, callBackParaValue);
