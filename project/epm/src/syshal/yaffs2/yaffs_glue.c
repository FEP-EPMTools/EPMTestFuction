...
const char *yaffs_error_str(void)
{
...
void cmd_yaffs_mread_fileByLen(char *fn, char *addr, int len)
{
    int h;
    struct yaffs_stat s;
    ...
    yaffs_read(h, addr, len);
    sysprintf("[cmd_yaffs_mread_file DONE, s.st_size = %d]\n", len);
...
void cmd_yaffs_mread_file(char *fn, char *addr)
{
    ...
    sysprintf("[cmd_yaffs_mread_file DONE(%s), s.st_size = %d (0x%x)]\n", fn, (int)s.st_size, s.st_size);
