...
#define CURRENT_TIME_ZONE                       8  
#define GMT_TIME_ZONE_DIFF_SECONDS              (60*60*CURRENT_TIME_ZONE)  

/*-----------------------------------------*/
/* global file scope (static) variables    */
/*-----------------------------------------*/
static UartInterface* pUartInterface = NULL;
static TSReaderInterface mReaderInterface = {OctopusReaderInit, OctopusReaderSetPower, OctopusReaderBreakCheckReader, OctopusReaderCheckReader, OctopusReaderProcess, NULL, OctopusReaderGetBootedStatus, OctopusReaderSignOnProcess, OctopusReaderSaveFile, OctopusReaderSaveFilePure};
...
static BOOL GetVerFlag = FALSE;

static BOOL HangUpStatus = FALSE;

static char GetVerStrBuf[64];
...
{  
    ...
    */
    ...
    if (s32_ret == ERR_NOERR) {
        ...
        memset(GetVerStrBuf, 0, sizeof(GetVerStrBuf));
        sprintf(GetVerStrBuf,"%u", DeviceV.FIRMVer);
        //verStrLen = sizeof(DeviceV.FIRMVer);
...
void OctopusReaderQueryTimeEx(UINT* tempYear,UINT* tempMonth,UINT* tempDay,UINT* tempHour,UINT* tempMinute,UINT* tempSecond)
{
    //stDevVer DeviceV;
    char timeStr[32];
    char* timeStrStart;
    ...
    //TimeVer((BYTE *) &DeviceV, &u32_tempstr[0]);
    
    //CmdTimeVer (&u32_tempstr[0]);
    
    *tempYear   = u32_tempstr[5];
    *tempMonth  = u32_tempstr[4];
    *tempDay    = u32_tempstr[3];
    *tempHour   = u32_tempstr[2];
    *tempMinute = u32_tempstr[1];
    *tempSecond = u32_tempstr[0];
    
    terninalPrintf ("20%d/%d/%d  %d:%d:%d\r\n", u32_tempstr[5], u32_tempstr[4], u32_tempstr[3], u32_tempstr[2], u32_tempstr[1], u32_tempstr[0]);
    
}


void OctopusReaderQueryTime(void)
{
    //stDevVer DeviceV;
    char timeStr[32];
    char* timeStrStart;
...
BOOL OctopusReaderInit(void)
{
    ...
    //terninalPrintf("ioctlFunc(UART_IOC_SET_OCTOPUS_MODE, 0, 0)\r\n");
    ...
    //AI[0] = 0;//GetMeterStorageData()->octopusReceiptNumber[0];
    //AI[1] = 0;//GetMeterStorageData()->octopusReceiptNumber[1];
...
uint8_t OctopusReaderCheckReader(void)
{
    ...
    sysprintf(" !!! OctopusReaderCheckReader enter  !!!...\r\n");
...
BOOL OctopusReaderCheckHangUpEnable(void)
{
    return CardReaderCheckHangUpEnable();
}


void InitOctopusReaderHangUpStatus(void)
{
    HangUpStatus = FALSE;
}

void OctopusReaderCheckHangUp(BOOL HUstatus)
{
    //return TSREADER_CHECK_READER_ERROR;
    HangUpStatus = HUstatus;
}

BOOL OctopusReaderHangUpStatus(void)
{
    return HangUpStatus;
}
...
void OctopusReaderGetVersion(char* ReaderVerBuf)
{      
    for(int i = 0; i<64; i++)
    {     
        ReaderVerBuf[i] = GetVerStrBuf[i];
        
    }
 
}

void ChangeOctopusKey(BOOL OctopusKey)
{
    if(OctopusKey == OCTOPUS_USE_PRODUCTION_KEY)
        ChangeKey(OCTOPUS_USE_PRODUCTION_KEY);
    else if(OctopusKey == OCTOPUS_USE_TEST_KEY)
        ChangeKey(OCTOPUS_USE_TEST_KEY);
}
