...
{   
    //terninalPrintf(" com_out  %d bytes..\n", len);
    //printfBuffData(" -- [com_out] --", msg, len);
    /*
    terninalPrintf("*msg => ");
    for(int i=0 ; i<len ; i++)
        terninalPrintf("%02x ", msg[i]);
    terninalPrintf("\n");
    */
    vTaskDelay(10/portTICK_RATE_MS);
    if(len > 0 )
    {
        if(OctopusReaderCheckHangUpEnable())
            outMsgFlag = TRUE;
        else
            outMsgFlag = FALSE;
    }
    ...
    GetTime(CMD_START);
    OctopusReaderWrite(msg, len); 
    
   // write(hComPort, msg, len);
    //tcdrain(hComPort);
...
INT ReadData (BYTE *Msg, INT iExpect, TickType_t* st_TimeOut)
{
    ...
    while(1)
    {
        ...
        /*
        terninalPrintf("<= ");
        for(int j=0 ; j<iExpect ; j++)
            terninalPrintf("%02x ", Msg[j]);
        terninalPrintf("\n");
        */
...
//    BYTE responseTmp[128];
    ...
    while((state!=ST_NAK)&&(state!=ST_COMPLETE))
    {
        ...
        if(state!=ST_SENDNAK)
        {
            ...
            if(result==0)
            {
                ...
                {
                    if(OctopusReaderCheckHangUpEnable())
                        OctopusReaderCheckHangUp(outMsgFlag);
                    else
                        outMsgFlag = FALSE;
                }
                //terninalPrintf("<= NoMsg\n");
                return 0;
            }
            else
                outMsgFlag = FALSE;
    ...
    /*
    terninalPrintf("<= ");
    for(int j=0 ; j<tempoint ; j++)
        terninalPrintf("%02x ", tempBuf[j]);
    terninalPrintf("\n");
    */
    vTaskDelay(10/portTICK_RATE_MS);
    if(tempoint > 0)
