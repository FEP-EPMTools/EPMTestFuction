...
static unsigned char* cJSONUtils_strdup(const unsigned char* str)
{
    ...
    if (!(copy = (unsigned char*)(uint8_t *)pvPortMalloc(len)))
    {
        return NULL;
    }
    memcpy(copy, str, len);
    ...
    for (obj = object->child; obj; obj = obj->next, c++)
    {
        ...
        if (found)
        {
            if ((type & 0xFF) == cJSON_Array)
            {
                ...
                unsigned char *ret = (unsigned char*)(uint8_t *)pvPortMalloc(strlen((char*)found) + 23);
            ...
            {
                unsigned char *ret = (unsigned char*)(uint8_t *)pvPortMalloc(strlen((char*)found) + cJSONUtils_PointerEncodedstrlen((unsigned char*)obj->string) + 2);
                *ret = '/';
...
static void cJSONUtils_GeneratePatch(cJSON *patches, const unsigned char *op, const unsigned char *path, const unsigned char *suffix, cJSON *val)
{
    ...
    if (suffix)
    {
        unsigned char *newpath = (unsigned char*)(uint8_t *)pvPortMalloc(strlen((const char*)path) + cJSONUtils_PointerEncodedstrlen(suffix) + 2);
...
static void cJSONUtils_CompareToPatch(cJSON *patches, const unsigned char *path, cJSON *from, cJSON *to)
{
    ...
    switch ((from->type & 0xFF))
    {
        ...
        {
            size_t c = 0;
            unsigned char *newpath = (unsigned char*)(uint8_t *)pvPortMalloc(strlen((const char*)path) + 23); /* Allow space for 64bit int. */
        ...
        case cJSON_Object:
        {
            ...
            while (a || b)
            {
                ...
                if (!diff)
                {
                    ...
                    unsigned char *newpath = (unsigned char*)(uint8_t *)pvPortMalloc(strlen((const char*)path) + cJSONUtils_PointerEncodedstrlen((unsigned char*)a->string) + 2);
