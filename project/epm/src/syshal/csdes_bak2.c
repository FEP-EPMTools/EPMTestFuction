...
/* Purpose     : encrypt/decrypt utility																		 */
...
static const ULONG des_SPtrans[8][64]={
{
        /* nibble 0 */
...
static const ULONG des_skb[8][64]={
{
...
static void des_encrypt (ULONG *data,    /* data to encrypt */
                         des_key_schedule ks,    /* key schedule */
                         INT encrypt);           /* encrpyt or decrypt flag */

                     /* DES ECB encryption */
static void des_ecb_encrypt (des_cblock *input,      /* input 8-byte block */
                             des_cblock *output,     /* output 8-byte block */
                             des_key_schedule ks,    /* key schedule */
                             INT encrypt)            /* encrypt or decrypt flag */
{
...
/* actual DES ECB encryption */
static void des_encrypt (ULONG *data,    /* data to encrypt */
                         des_key_schedule ks,    /* key schedule */
                         INT encrypt)            /* encrpyt or decrypt flag */
{
    register ULONG l, r, u;
    union fudge
    {
        ULONG  l;
        USHORT s[2];
...
void CscryptEncrypt (BYTE *key,         /* 8-byte key */
                     ...
                     INT encrypt)                /*	DES
                                                     1=encrypt, 0=decrypt
                                                     3DES
                                                     31=encrypt, 30=decrypt */
{
    BYTE b3Des=encrypt>=DES3CODE;
    ...
    des_ecb_encrypt ((des_cblock *)in,      /* input 8-byte block */
        (des_cblock *)out,     /* output 8-byte block */
                     ks,                     /* key schedule */
                     encrypt);               /* encrypt or decrypt flag */
    if(b3Des)
    {
        des_set_key ((des_cblock *)(key+8), ks);
        des_ecb_encrypt ((des_cblock *)out,  /* input 8-byte block */
            (des_cblock *)out,     /* output 8-byte block */
                         ks,                     /* key schedule */
                         1-encrypt);               /* encrypt or decrypt flag */
        des_set_key ((des_cblock *)key, ks);
        des_ecb_encrypt ((des_cblock *)out,  /* input 8-byte block */
            (des_cblock *)out,     /* output 8-byte block */
                         ks,                     /* key schedule */
                          ...
                          INT encrypt)                /* encrypt or decrypt */
{
    ...
    for(i=0; i<length; i+=8)
        CscryptEncrypt (key, &blk[i], &blk[i], encrypt);
}
